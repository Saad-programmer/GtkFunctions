...
void update_window(window *wn) {
   ...
   if (wn->icon_path) {
        GError *error = NULL;
        if (!gtk_window_set_icon_from_file(GTK_WINDOW(wn->win), wn->icon_path, &error)) {
            g_printerr("Failed to load icon from %s: %s\n", wn->icon_path, error->message);
            g_error_free(error);
        }
    }
    ...
    if(wn->bg_image){
        GtkWidget *bg_image = gtk_image_new_from_file(wn->bg_image);
        gtk_container_add(GTK_CONTAINER(wn->win), bg_image);
